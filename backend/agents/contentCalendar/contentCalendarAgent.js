...
3. Observe channel segregation:
   ...
   - EMAILS: Focus on Stage 1 (Awareness) and Stage 2 (Engagement).
   - WHATSAPP: Focus on Stage 1 (Awareness) and Stage 2 (Engagement) with mobile-first hook formats.
...
7. DAY-TO-DAY CONTINUITY (STRICT): The 15 days are one continuous campaign, not 15 isolated pieces. Treat each channel's own sequence (blogs vs blogs, emails vs emails, whatsapp vs whatsapp) as chapters of one story:
   ...
   - Stage 1 (Awareness) items should set up themes that Stage 2 (Engagement) items in the same channel then deepen or convert.
  ...
  const safeStringify = (value) => {
    ...
    try {
      return JSON.stringify(value, null, 2).substring(0, 14000);
    } catch (error) {
      return String(value).substring(0, 14000);
    }
  };
...
${JSON.stringify(CAMPAIGN_TOPOLOGY, null, 2)}
...
function weaveAdminApprovalPayload(llmStrategy, startDateISO) {
  ...
  return {
    ...
    summary: {
      ...
      activeFunnelStages: ["1_AWARENESS", "2_ENGAGEMENT"]
    },
    schedule: {
      websiteBlogs,
      emailMessages,
